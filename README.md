...
- Database schema được tự động tạo/update với `spring.jpa.hibernate.ddl-auto=update`
- Để production, nên sử dụng Flyway hoặc Liquibase cho database migration
- Cấu hình credentials nên được quản lý qua environment variables hoặc secrets manager
- Logging được cấu hình với SLF4J + Logback
